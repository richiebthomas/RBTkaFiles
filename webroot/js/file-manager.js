...
class FileManager {
    ...
    bindEvents() {
        ...
        $('#btn-refresh').on('click', () => this.loadDirectory(this.currentPath));
    ...
    loadDirectory(path) {
        ...
        const ajaxUrl = '/api/browse' + (path ? '/' + encodeURIComponent(path) : '');
    ...
    createFileItemElement(item) {
        ...
        const element = $(`
            <tr class="file-item ${item.type}" 
                ...
                <td class="file-type-cell">${typeText}</td>
    ...
    handleItemDoubleClick(e) {
        ...
        if (type === 'folder') {
            ...
            const previewUrl = '/api/preview/' + encodeURIComponent(path);
    ...
    uploadFiles(files) {
        ...
        for (let i = 0; i < files.length; i++) {
            if (this.isDangerousFile(files[i].name)) {
                dangerousFiles.push(files[i].name);
            } else {
                validFiles.push(files[i]);
            }
        }
        
        // Show error for dangerous files
        if (dangerousFiles.length > 0) {
            this.showError(
                `Security Error: The following file types are not allowed:\n\n` +
                dangerousFiles.join('\n') + '\n\n' +
                `Safe file types: ${this.getSafeFileTypes().slice(0, 8).join(', ')}...`
            );
        ...
        // Clear form data and add only valid files
        ...
        $.ajax({
            ...
            success: (response) => {
                if (response.success) {
                    let message = `Successfully uploaded ${validFiles.length} file(s)`;
                    if (typeof dangerousFiles !== 'undefined' && dangerousFiles.length > 0) {
                        message += ` (${dangerousFiles.length} file(s) blocked for security)`;
                    }
    ...
    handleUploadResponse(response) {
        if (response.success) {
            ...
            if (errorCount === 0) {
                this.showSuccess(`${successCount} files uploaded successfully`);
            } else {
                this.showWarning(`${successCount} files uploaded, ${errorCount} failed`);
            }
        }
    ...
    showPreview(filePath) {
        ...
        // Get file info
        $.ajax({
            url: '/api/file-info/' + encodeURIComponent(filePath),
            method: 'GET',
            dataType: 'json',
            success: (response) => {
    ...
    loadPreviewContent(file) {
        const previewUrl = '/api/preview/' + encodeURIComponent(file.path);
