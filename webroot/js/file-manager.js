class FileManager {
    ...
    bindEvents() {
        ...
        $('#btn-open-pad').on('click', () => this.openPad());
        ...
        $('#folder-name').on('keypress', (e) => {
            if (e.which === 13) $('#btn-create-folder-confirm').click();
        });
        $('#rename-input').on('keypress', (e) => {
            if (e.which === 13) $('#btn-rename-confirm').click();
        });
        $(document).on('click', '.read-more-btn', (e) => this.toggleReadMore(e));
        $(document).on('click', '#add-note', () => {
            const notesList = $('#notes-list');
            const newNoteElement = this.createNoteElement({
                id: 'new',
                content: '',
                created: new Date().toISOString()
            });
            notesList.prepend(newNoteElement);
            this.enterNoteEditMode(newNoteElement);
        });
        $(document).on('click', '.edit-note', (e) => {
            const noteElement = $(e.currentTarget).closest('.note-item');
        ...
        $(document).on('click', '.copy-note', (e) => {
            const noteElement = $(e.currentTarget).closest('.note-item');
            this.copyNoteContent(noteElement);
        });
        $(document).on('click', '.save-note', (e) => {
            const noteElement = $(e.currentTarget).closest('.note-item');
            this.exitNoteEditMode(noteElement, true);
        });
        $(document).on('click', '.cancel-edit', (e) => {
            const noteElement = $(e.currentTarget).closest('.note-item');
            this.exitNoteEditMode(noteElement, false);
        });
    ...
    setupFileDragAndDrop() {
        ...
        $('#file-listing').on('drop', (e) => {
            ...
            if (files && files.length > 0) {
                e.preventDefault();
                this.showWarning('Drag-to-upload is disabled. Use the Upload Files button.');
    ...
    handleDragEnter(e) {
        ...
        if (this.isValidDropTarget($target)) {
            $target.addClass('drag-over-target');
        }
    }
    ...
    handleDragLeave(e) {
        const $target = $(e.currentTarget);
        if (!$target.is(e.relatedTarget) && !$target.has(e.relatedTarget).length) {
            $target.removeClass('drag-over-target');
        }
    }
    ...
    handleDrop(e) {
        ...
        if ($target.hasClass('breadcrumb-item')) {
            targetPath = $target.find('a').data('path') || '';
        } else if ($target.is('a') && $target.parent().hasClass('breadcrumb-item')) {
            targetPath = $target.data('path') || '';
        } else if ($target.hasClass('file-item')) {
            targetPath = $target.data('path');
        }
        if (targetPath === this.draggedItem.path || this.dirname(this.draggedItem.path) === targetPath) {
            return;
        }
    ...
    isValidDropTarget($target) {
        ...
        if ($target.hasClass('breadcrumb-item') || ($target.is('a') && $target.parent().hasClass('breadcrumb-item'))) {
            return true;
        }
        if ($target.hasClass('file-item') && $target.data('type') === 'folder') {
            const targetPath = $target.data('path');
            if (this.draggedItem.type === 'folder' && (targetPath === this.draggedItem.path || targetPath.startsWith(this.draggedItem.path + '/'))) {
                return false;
            }
            return true;
        }
    ...
    loadDirectory(path) {
        ...
        const url = path ? `/${this.encodePath(path)}` : '/';
        window.history.pushState({
            path: path
        }, '', url);
        const ajaxUrl = '/api/browse' + (path ? '/' + this.encodePath(path) : '');
        $.ajax({
            url: ajaxUrl,
            method: 'GET',
            dataType: 'json',
            success: (response) => {
                if (response.success) {
                    this.renderDirectory(response);
                } else {
                    this.showError('Failed to load directory: ' + (response.message || 'Unknown error'));
                }
            },
            error: (xhr, status, error) => {
                this.showError('Failed to load directory: ' + xhr.statusText);
                this.hideLoading();
            }
        });
    }
    ...
    renderDirectory(response) {
        ...
        if (response.notes !== undefined) {
            this.showNotes(this.currentNotes);
        } else {
            this.hideNotes();
        }
        this.setupFileDragAndDrop();
    ...
    getFileIcon(filename, mimeType) {
        ...
        if (['js', 'html', 'css', 'php', 'py', 'java', 'cpp', 'c', 'cs', 'rb', 'go'].includes(ext)) {
    ...
    showCreateFolderModal() {
        $('#folder-name').val('');
        $('#folder-name-error').hide();
        $('#createFolderModal').modal('show');
        setTimeout(() => $('#folder-name').focus(), 500);
    }
    ...
    createFolder() {
        ...
        $.ajax({
            ...
            error: (xhr) => {
                this.showModalError('#folder-name-error', 'Failed to create folder: ' + xhr.statusText);
            }
        });
    }
    ...
    showContextMenu(e) {
        ...
        $('#ctx-download').toggle(this.selectedItem.type === 'file');
        const isPdf = this.selectedItem.type === 'file' && this.selectedItem.name.toLowerCase().endsWith('.pdf');
        $('#ctx-print').toggle(isPdf);
    ...
    showRenameModal() {
        ...
        $('#rename-input').val(baseName);
        $('#rename-error').hide();
        if (this.selectedItem.type === 'file' && extension) {
            $('#rename-extension-info').html(`
    ...
    renameItem() {
        ...
        if (!newName) {
            this.showModalError('#rename-error', 'Name is required');
            return;
        }
        if (this.selectedItem.type === 'file') {
            const currentName = this.selectedItem.name;
        ...
        $.ajax({
            ...
            error: (xhr) => {
                this.showModalError('#rename-error', 'Failed to rename: ' + xhr.statusText);
            }
        });
    }
    ...
    deleteItem() {
        if (!this.selectedItem) return;
        let message = `Are you sure you want to delete "${this.selectedItem.name}"?`;
        ...
        $.ajax({
            ...
            error: (xhr) => {
                this.showError('Failed to delete: ' + xhr.statusText);
            }
        });
        this.hideContextMenu();
    }
    ...
    handleKeyboard(e) {
        if (e.keyCode === 46 && this.selectedItem) {
            this.deleteItem();
        }
        if (e.keyCode === 113 && this.selectedItem) {
    ...
    showModalError(selector, message) {
    ...
    handlePrintSubmit(e) {
        ...
        formData.append('file', this.selectedItem.path);
        formData.append('name', $('#print-name').val().trim());
        formData.append('roll', $('#print-roll').val().trim());
        formData.append('lab', $('#print-lab').val().trim());
        const name = $('#print-name').val().trim();
        const roll = $('#print-roll').val().trim();
        if (!name || !roll) {
            this.showPrintError('Name and Roll Number are required.');
            return;
        }
    ...
    handleRollNumberInput(e) {
        const rollNumber = $(e.target).val().trim();
        $('#user-status').hide();
        $('#print-error').hide();
        if (rollNumber.length === 7 && /^\d{7}$/.test(rollNumber)) {
            this.lookupUser(rollNumber);
        } else if (rollNumber.length > 7) {
            $(e.target).val(rollNumber.substring(0, 7));
        }
    }

    lookupUser(rollNumber) {
    $.ajax({
        url: '/api/lookup-user',
        method: 'POST',
        ...
        success: (response) => {
            if (response.success && response.user_found) {
                $('#print-name').val(response.user.name);
                $('#user-status').show().find('small').html('<i class="fas fa-check-circle"></i> User found! Name auto-filled.').removeClass('text-warning text-danger').addClass('text-success');
            } else if (response.success && !response.user_found) {
                $('#print-name').val('').focus();
                $('#user-status').show().find('small').html('<i class="fas fa-info-circle"></i> New user - please enter your name.').removeClass('text-success text-danger').addClass('text-warning');
            }
        },
        error: (xhr) => {
            $('#user-status').show().find('small').html('<i class="fas fa-exclamation-triangle"></i> Error looking up user.').removeClass('text-success text-warning').addClass('text-danger');
            $('#print-name').focus();
        }
    });
}
...
isDangerousFile(filename) {
    const dangerousExtensions = ['exe', 'bat', 'cmd', 'com', 'pif', 'scr', 'vbs', 'vbe', 'js', 'jse', 'jar', 'msi', 'msp', 'hta', 'cpl', 'msc', 'wsf', 'wsh', 'ps1', 'ps2', 'psc1', 'psc2', 'php', 'asp', 'aspx', 'jsp', 'pl', 'py', 'rb', 'sh', 'cgi', 'htaccess', 'htpasswd', 'ini', 'cfg', 'conf', '7z', 'tar', 'gz', 'bz2', 'xz', 'sql', 'db', 'sqlite', 'mdb'];
    const extension = filename.toLowerCase().split('.').pop();
    return dangerousExtensions.includes(extension);
}

getSafeFileTypes() {
    return ['pdf', 'doc', 'docx', 'xls', 'xlsx', 'ppt', 'pptx', 'odt', 'ods', 'odp', 'jpg', 'jpeg', 'png', 'gif', 'bmp', 'tiff', 'svg', 'webp', 'txt', 'rtf', 'csv', 'xml', 'json', 'mp3', 'wav', 'ogg', 'flac', 'aac', 'm4a', 'mp4', 'avi', 'mkv', 'mov', 'wmv', 'flv', 'webm', 'log', 'md', 'readme'];
}
...
getFileType(filename) {
    ...
    return typeMap[extension] || (extension ? extension.toUpperCase() + ' File' : 'File');
}
...
showPreview(filePath) {
    ...
    $.ajax({
        ...
        success: (response) => {
            if (response.success) {
                this.renderPreview(response.file);
            } else {
                this.showPreviewError('Failed to load file information: ' + (response.message || 'Unknown error'));
            }
        },
        error: (xhr, status, error) => {
            let errorMessage = 'Failed to load file: ' + xhr.statusText;
            try {
...
showPdfPreview(previewUrl, file) {
    $('#preview-body').html(`
...
showTextPreview(previewUrl, file) {
    $.ajax({
        url: previewUrl,
        method: 'GET',
        dataType: 'text',
...
isTextFile(file) {
    ...
    return file.mime_type.startsWith('text/') || ['application/json', 'application/xml', 'application/javascript'].includes(file.mime_type) || ['txt', 'md', 'json', 'xml', 'html', 'htm', 'css', 'js', 'php', 'py', 'java', 'c', 'cpp', 'sql'].includes(file.extension);
}
...
showNotes(notes) {
    ...
    const notesList = notesSection.find('#notes-list');
    notesSection.show();
    notesList.empty();
    
    if (Array.isArray(notes) && notes.length > 0) {
        notes.forEach(note => {
            notesList.append(this.createNoteElement(note));
        });
    } else {
        notesList.html('<p class="text-muted text-center">No notes yet. Click "Add Note" to create one.</p>');
    }
}

createNoteElement(note) {
    const date = new Date(note.modified || note.created);
    const formattedDate = date.toLocaleString();
    const contentLength = note.content.length;
    const needsReadMore = contentLength > 100;
        ...
        <div class="note-item" data-note-id="${note.id}">
            <div class="d-flex justify-content-between align-items-start">
                ...
                <div class="note-actions">
                    <button class="btn btn-sm btn-outline-secondary copy-note" title="Copy">
...
enterNoteEditMode(noteElement) {
    noteElement.find('.note-content, .note-actions').hide();
    noteElement.find('.note-editor, .edit-actions').show();
    noteElement.find('.note-editor').focus();
}

exitNoteEditMode(noteElement, save = false) {
    const noteId = noteElement.data('note-id');
    const editor = noteElement.find('.note-editor');
    const content = editor.val().trim();
    
    if (save) {
        const action = (noteId === 'new') ? 'add' : 'edit';
        this.saveNote({ noteId: noteId, content: content, action: action });
    } else {
        noteElement.find('.note-editor, .edit-actions').hide();
        noteElement.find('.note-content, .note-actions').show();
    }
}
...
saveNote(data) {
    ...
    $.ajax({
        url: '/api/save-notes',
        method: 'POST',
        data: { path: this.currentPath, noteId: data.noteId, content: data.content, action: data.action },
        success: (response) => {
            if (response.success) {
...
deleteNote(noteId) {
    ...
    $.ajax({
        url: '/api/save-notes',
        method: 'POST',
        data: { path: this.currentPath, noteId: noteId, action: 'delete' },
        success: (response) => {
            if (response.success) {
                this.showNotes(response.notes);
...
copyNoteContent(noteElement) {
    const noteContent = noteElement.find('.note-content').text();
    
    if (navigator.clipboard && window.isSecureContext) {
        navigator.clipboard.writeText(noteContent).then(() => {
            this.showSuccess('Note content copied to clipboard');
        }).catch(() => {
            this.fallbackCopyTextToClipboard(noteContent);
        });
    } else {
        this.fallbackCopyTextToClipboard(noteContent);
    }
...
fallbackCopyTextToClipboard(text) {
    ...
    try {
        ...
        if (successful) {
            ...
            this.showError('Failed to copy note content');
        }
    } catch (err) {
        this.showError('Failed to copy note content');
    }
...
toggleReadMore(event) {
    ...
    const noteContent = button.siblings('.note-content');
    const currentState = button.attr('data-state');
    
    if (currentState === 'collapsed') {
        noteContent.removeClass('collapsed');
        button.attr('data-state', 'expanded');
        button.text('Show Less');
    } else {
        noteContent.addClass('collapsed');
...
handleSearchBlur(e) {
    ...
    if (!$suggestions.is(e.relatedTarget) && !$suggestions.has(e.relatedTarget).length) {
        setTimeout(() => {
            this.hideSearchSuggestions();
        }, 200);
    }
...
handleSearchKeydown(e) {
    const $suggestions = $('#search-suggestions');
    const $active = $suggestions.find('.search-suggestion.active');
    ...
    if (e.key === 'ArrowDown') {
        ...
        if ($active.length) {
            ...
            $suggestions.find('.search-suggestion:last').addClass('active');
        }
    } else if (e.key === 'Enter') {
        e.preventDefault();
        if ($active.length) {
...
handleSuggestionClick(e) {
    ...
    const item = $suggestion.data('item');
    ...
    if (item.type === 'folder') {
        ...
        const notePath = item.path || '';
        this.loadDirectory(notePath);
        this.hideSearchSuggestions();
        $('#search-input').val('');
        setTimeout(() => {
        ...
        const parentPath = this.dirname(item.path);
        this.loadDirectory(parentPath);
        this.hideSearchSuggestions();
        $('#search-input').val('');
        setTimeout(() => {
            $(`.file-item[data-path="${item.path}"]`).click();
        }, 100);
    }
}
    ...
    $.ajax({
        ...
        success: (response) => {
            if (response.success) {
                ...
                this.showError('Search error: ' + (response.message || 'Unknown error'));
            }
        },
        error: (xhr) => {
            this.showError('Search failed: ' + xhr.statusText);
        }
    });
}
...
renderSearchSuggestions(suggestions) {
    ...
    if (suggestions.length === 0) {
        $container.html('<div class="search-suggestion no-results"><i class="fas fa-search"></i> No results found</div>');
        return;
    }
...
getFileIconClass(item) {
    if (item.type === 'note') {
        ...
        return iconMap[extension] || 'fas fa-file file-icon-text';
    }
}
...
scrollToNotesSection() {
    $('#notes-section').show();
    $('html, body').animate({
        scrollTop: $('#notes-section').offset().top - 100
    }, 500);
}
...
$(document).ready(function() {
    try {
        ...
        window.addEventListener('popstate', function(e) {
            ...
            window.fileManager.currentPath = path;
            window.fileManager.loadDirectory(path);
        });
    } catch (error) {}
});