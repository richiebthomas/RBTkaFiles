class FileManager{constructor(){this.currentPath=window.currentPath||'';this.selectedItem=null;this.notesVisible=!1;this.currentNotes='';this.hoverTimeout=null;this.hoverHideTimeout=null;this.isProcessingMove=!1;this.searchTimeout=null;this.searchVisible=!1;this.init()}
...
showContextMenu(e){e.preventDefault();const $item=$(e.currentTarget);this.selectedItem={path:$item.data('path'),type:$item.data('type'),name:$item.find('.file-name').text()};$('#ctx-download').toggle(this.selectedItem.type==='file');const isPdf=this.selectedItem.type==='file'&&this.selectedItem.name.toLowerCase().endsWith('.pdf');$('#ctx-print').toggle(isPdf);const $menu=$('#context-menu');const menuWidth=200;const menuHeight=150;const viewportWidth=$(window).width();const viewportHeight=$(window).height();let left=e.clientX;let top=e.clientY;if(left+menuWidth>viewportWidth){left=viewportWidth-menuWidth-10}if(top+menuHeight>viewportHeight){top=viewportHeight-menuHeight-10}if(left<10){left=10}if(top<10){top=10}$menu.css({display:'block',left:left,top:top});$('.file-item').removeClass('selected');$item.addClass('selected')}
...
copyNoteContent(noteElement){const noteContent=noteElement.find('.note-content').text();if(navigator.clipboard&&window.isSecureContext){navigator.clipboard.writeText(noteContent).then(()=>{this.showSuccess('Note content copied to clipboard')}).catch(()=>{this.fallbackCopyTextToClipboard(noteContent)})}else{this.fallbackCopyTextToClipboard(noteContent)}}
fallbackCopyTextToClipboard(text){const textArea=document.createElement('textarea');textArea.value=text;textArea.style.position='fixed';textArea.style.left='-999999px';textArea.style.top='-999999px';document.body.appendChild(textArea);textArea.focus();textArea.select();try{const successful=document.execCommand('copy');if(successful){this.showSuccess('Note content copied to clipboard')}else{this.showError('Failed to copy note content')}}catch(err){this.showError('Failed to copy note content')}document.body.removeChild(textArea)}
